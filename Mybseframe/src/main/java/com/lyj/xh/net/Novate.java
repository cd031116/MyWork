...
import com.lyj.xh.net.cache.CookieCacheImpl;
import com.lyj.xh.net.cache.SharedPrefsCookiePersistor;
import com.lyj.xh.net.config.ConfigLoader;
import com.lyj.xh.net.cookie.NovateCookieManager;
import com.lyj.xh.net.dowload.DownLoadCallBack;
import com.lyj.xh.net.dowload.DownSubscriber;
import com.lyj.xh.net.exception.FormatException;
import com.lyj.xh.net.exception.NovateException;
import com.lyj.xh.net.exception.ServerException;
...
public class Novate {
    ...
    private class HandleFuc<T> implements Func1<NovateResponse<T>, T> {
        @Override
        public T call(NovateResponse<T> response) {//response.getResult() == null &&
...
*/
    ...
    public <T> T isNullBody(final String url, final ResponseCallBack<T> callBack) {
        final Type[] types = callBack.getClass().getGenericInterfaces();
        if (MethodHandler(types) == null || MethodHandler(types).size() == 0) {
            return null;
        }
        ...
        return (T) apiManager.isNullBody(url,null)
                .compose(schedulersTransformer)
                .compose(handleErrTransformer())
                .subscribe(new NovateSubscriber<T>(mContext, finalNeedType, callBack));
    }
    ...
    class NovateSubscriber<T> extends BaseSubscriber<ResponseBody> {
        ...
        public void onNext(ResponseBody responseBody) {
            try {
                ...
                Log.i("OkHttp", "ResponseBody:" + jsStr);
                if (callBack != null) {
                    try {
                        /**
                        ...
                        baseResponse = new Gson().fromJson(jsStr, finalNeedType);// baseResponse.getData() == null &
