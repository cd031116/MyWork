...
public class NovateException {
    ...
    public static Throwable handleException(java.lang.Throwable e) {
        ...
        if (e instanceof HttpException) {
            ...
            ex = new Throwabled(e, ERROR.HTTP_ERROR);
            ...
            ex = new Throwabled(resultException, resultException.code);
            ex.setMessage(resultException.getMessage());
            return ex;
        } else if (e instanceof JsonParseException
                || e instanceof JSONException
                || e instanceof ParseException) {
            ex = new Throwabled(e, ERROR.PARSE_ERROR);
            ex.setMessage("解析错误");
            return ex;
        } else if (e instanceof ConnectException) {
            ex = new Throwabled(e, ERROR.NETWORD_ERROR);
            ex.setMessage("连接失败");
            return ex;
        } else if (e instanceof javax.net.ssl.SSLHandshakeException) {
            ex = new Throwabled(e, ERROR.SSL_ERROR);
            ex.setMessage("证书验证失败");
            return ex;
        } else if (e instanceof java.security.cert.CertPathValidatorException) {
            ex = new Throwabled(e, ERROR.SSL_NOT_FOUND);
            ex.setMessage("证书路径没找到");
            return ex;
        }
        else if (e instanceof ConnectTimeoutException){
            ex = new Throwabled(e, ERROR.TIMEOUT_ERROR);
            ex.setMessage("连接超时");
            return ex;
            ...
            ex = new Throwabled(e, ERROR.FORMAT_ERROR);
            ex.setMessage("类型转换出错");
            return ex;
        } else if (e instanceof NullPointerException) {
            ex = new Throwabled(e, ERROR.NULL);
            ex.setMessage("数据有空");
            return ex;
            ...
            ex = new Throwabled(e, ERROR.UNKNOWN);
